...
namespace QtNodes {
//Takes a shared pointer to a model registry (NodeDelegateModelRegistry) and stores it.
// Initializes the next node ID counter to 0.
//This class relies on a registry to create node models by type.
DataFlowGraphModel::DataFlowGraphModel(std::shared_ptr<NodeDelegateModelRegistry> registry)
    : _registry(std::move(registry))
    , _nextNodeId{0}
{}
// Returns all existing NodeIds by iterating through _models, which maps node IDs to their models.
