...
inline void initResources() // Initializes compiled-in Qt resources (e.g., :DefaultStyle.json).
// This is a workaround for the static initialization order problem in C++.
{
...
{
    QJsonObject obj;
// Writes each value into the JSON using macros.
...
// Wraps in a root object and returns it.
    QJsonObject root;
...
//Generates a unique hue per typeId (e.g., a data type like "int", "image"), Ensures different types get visually distinct colors.
...
//Returns line width, construction width, and circle point size.
...
//Returns whether node port colors should be determined by their data types instead of fixed color.
