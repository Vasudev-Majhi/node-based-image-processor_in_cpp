...
public:
    ...
    std::unordered_map<NodeId, QVariant> _nodeInternalData; // Stores internal data (e.g., images) for nodes
   ...
   */
    ...
    void loadImage(NodeId nodeId); // Load image for a specific node
...
private:
    ...
    std::unordered_map<NodeId, cv::Mat> _nodeImages; // Store images for each node
